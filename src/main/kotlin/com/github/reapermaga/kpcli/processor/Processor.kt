...
val processors: Array<Processor<out WizardResult>> = arrayOf(
    GradleProcessor()
 )

interface Processor<T : WizardResult> {
    val wizardResultType: KClass<T>
    fun process(template: Template, folder: File, wizardResult: T)

    @Suppress("UNCHECKED_CAST")
    fun unsafeProcess(template: Template, folder: File, wizardResult: WizardResult) {
