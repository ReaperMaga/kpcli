...
val supportedDependencies = mapOf(
...
class GradleProcessor : Processor<GradleWizardResult> {
    ...
    override fun process(template: Template, folder: File, wizardResult: GradleWizardResult) {
        val buildGradleKtsFile = folder.listFiles()?.firstOrNull { it.name == "build.gradle.kts" } ?: error("Couldn't find build.gradle.kts")
        ...
        if(text.contains(pluginsComment)) {
            text = text.replace(pluginsComment, buildString {
                plugins.forEach {
        ...
        if(text.contains(dependenciesComment)) {
            text = text.replace(dependenciesComment, buildString {
        ...
        if(text.contains(repositoriesComment)) {
            text = text.replace(repositoriesComment, buildString {
                repositores.forEach {
