...
class BaseWizardResult(
    ...
    override fun toString(): String {
        return "BaseWizardResult: [deps: ${dependencies.joinToString(", ")}]"
    }
}
...
class BaseWizard : Wizard<BaseWizardResult> {
    override fun prompt(prompt: ConsolePrompt): BaseWizardResult {
        ...
        builder.createCheckboxPrompt()
            .name("dependencies").message("Please select the dependencies you want to include:")
            .newItem().name("ktlint").text("Ktlint").add()
            .newItem().name("reapermaga-common-library").text("ReaperMaga's Common Library").add()
            .addPrompt()
